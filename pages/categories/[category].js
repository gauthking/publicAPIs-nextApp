...
export async function getStaticPaths() {
    ...
    const paths = data.entries.map((ele) => {
        return {
            params: {
                category: ele.Category.toLowerCase(),
            },
        };
    });
...
export async function getStaticProps({ params }) {
    ...
    console.log(cat)
    const res = await fetch(`https://api.publicapis.org/entries?category=${cat}`);
    const data = await res.json();
            ...
            <div className={`navbar flex ${toggle === true ? "bg-gray-900" : "bg-green-300"} p-8 items-center rounded-b-2xl m-4 rounded-t-xl gap-10 shadow-blue-200 drop-shadow-sm`}>
                ...
                <div className="flex justify-center items-center ml-auto">
                    <label className="inline-flex relative items-center cursor-pointer">
                        ...
                        <span className={`ml-3 text-sm font-medium text-black`}>Dark Mode</span>
                    </label>
                </div>
            </div >
                            ...
                            <div className="h-full border-2 border-gray-800 rounded-lg overflow-hidden w-full shadow-gray-500  shadow-md hover:scale-110 transition-all ease-in-out">
                                <div className="p-6">
                                    ...
                                    <div className="flex items-center flex-wrap ">
                                        <a href={`/learnmore/${(entry.API).toLowerCase()}`} className="text-indigo-400 hover:text-indigo-600 transition-all ease-in inline-flex items-center md:mb-2 lg:mb-0 cursor-pointer">Learn More
